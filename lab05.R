...
The above lines fails because you cannot invert a rank difficient matrix. It is computationally singular.
...
It is expected to be different from 1. 
...
Infinite amount of orthonormal basis and they are not the same. 
...
```{r}
mod = lm(ytrain ~ . + 0, data.frame(xtrain))
sd(mod$residuals) 
y_hat = predict(mod, data.frame(xtest))
e = y_test - y_hat
oosSE = sd(e)
oosSE
```
